...
const generateKey = (pass: string) => {
  ...
  var derivedParams = JsCrypto.OpenSSLKDF.execute(pass, keySize, ivSize, salt);

  JsCrypto.Word32Array.random;
...
const generatePatientKey = () => {
  ...
  const seed = JsCrypto.Word32Array.random(seedSize / 8);

  const key = CryptoJS.PBKDF2(seed.toString(), salt.toString(), {
    keySize: keySize / 32,
...
const encryptWithKey = (msg: string, key: any) => {
  const iv = JsCrypto.Word32Array.random(ivSize / 8);
  ...
  var encryptedData = JsCrypto.AES.encrypt(msg, key.value, {
    iv,
  ...
  const transitmessage = key.iv.toString() + encryptedData.toString();
...
const decryptWithKey = (transitmessage: string, key: any) => {
  const iv = JsCrypto.Hex.parse(transitmessage.substr(0, 32));
  const encrypted = transitmessage.substring(32);

  var decryptedData = JsCrypto.AES.decrypt(encrypted, key.value, {
    iv,
    mode: JsCrypto.mode.GCM,
  });
